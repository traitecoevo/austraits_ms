...
p_aus_try <-
  aus_try_comparison %>% 
  mutate_at(c("n_austraits", "n_try"), ~.x/26000*100) %>% 
  ...
  geom_abline(intercept = -4, slope = 1, alpha = 0.2) +
  theme_classic() +
  scale_x_log10(limits =c(1E-3, 120), breaks = c(0.001, 0.01, 0.1,1, 10, 100), labels = function(.x) scales::comma(.x, accuracy = 0.01), expand= expansion(0)) +
  scale_y_log10(limits =c(1E-3, 120), breaks = c(0.001, 0.01, 0.1,1, 10, 100), labels = function(.x) scales::comma(.x, accuracy = 0.01), expand= expansion(0)) +
