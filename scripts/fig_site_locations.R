...
## Filter sites
sites %>% 
  filter(`latitude (deg)` > (-45),
         `latitude (deg)` < (-9.5),
         `longitude (deg)` < (153),
         `longitude (deg)` > (110)) %>% 
  filter(!site_name %in%c("site_at_-17.4167_degS_and_151.8833_degE",
                          "site_at_-16.5833_degS_and_150.3167_degE",
                          "site_at_-16.9333_degS_and_149.1833_degE",
                          "site_at_-16.9833_degS_and_149.8833_degE")) -> sites2



#### 03 Overlay site locations  ####
austraits_site_locations <- au_basemap +
  geom_pointdensity(data = sites2,
                    aes(y = `latitude (deg)`,
                        x = `longitude (deg)`
                    ),
                    inherit.aes = FALSE,
  ...
  scale_fill_grey(name="", start = 0.8,guide=FALSE, na.value = "white")
