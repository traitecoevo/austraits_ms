...
bibliography: [data/references_all.bib]
---
...
sources_all <- 
  bibtex::read.bib("data/references_ms.bib")

RefManageR::WriteBib(austraits$sources, "data/references_data.bib")

c(readLines("data/references_ms.bib"), "\n", readLines("data/references_data.bib")) %>% writeLines("data/references_all.bib")
  ...
  austraits$contributors %>% 
    filter(
          ...
          !dataset_id %in% c("Lawes_2012", "Westman_1977", "Hall_1981", "Hocking_1982", "Hocking_1986", "Toelken_1996", "Brock_1993", "Forster_1992", "Forster_1995", "Goble_1981"),
           ...
           !name %in% c("Huw Morgan", "Wendy Cooper", "Muhammad Islam", "Michael Lawes")
           ) %>% 
...
numbers of taxa / figures

* AusTraits includes more seed and fruit nutrient data; possibly reflecting the interest in Australia, specifically WA, in understanding how fruit and seeds are provisioned in nutrient depauperate environments
* AusTraits includes more categorical values, especially variables documenting different components of species’ fire response strategies, reflecting the importance of fire in shaping Australian communities and the research to document different strategies species have evolved to succeed in fire-prone environments

- On the point about many Australian species being unrepresented in global trait databases (end of the second paragraph in Background & Summary) -- I was wondering if this could be exemplified by reporting the number of species in AusTraits compared to the number of Australian species in the TRY Plant Database (in order to show that AusTraits fills this gap substantially).
- I was also wondering if the number of studies that have used global plant trait databases could be reported as an example to show how valuable AusTraits will be (e.g. TRY Plant Trait Database having 1613 citations according to Google Scholar) (relevant to the point that is made in the second-last sentence of Background & Summary).
...
AusTraits was assembled from `r austraits$sources %>% length()` distinct sources, including published papers, field campaigns, botanical collections, and taxonomic treatments. Initially we identified a list of candidate traits of interest -- including growth form, plant height, leaf size, specific leaf area and seed mass -- then identified primary sources containing measurements for these trait, before contacting authors for access. As the compilation grew, we expanded the list of traits considered to include any measureable quantity that had been quantified for a moderate number of taxa ( n > 20). A full list of traits and their sources appears in Table 1 (available online only). Details from each source were saved with minimal modification into two plain text files:
...
sprintf("@%s",keys) %>% paste(collapse = "; ") %>% paste0("[", ., "]") %>% writeLines()
```
...
Taxonomy updated via Australian Plant Census (APC) [@apc-2020], Australian Plant Names Index (APNI) [@apni-2020]

#### Schema


Each original source was designated a `dataset_id`. Ordinarily the id is the surname of first author and year of publication, for the source’s primary citation (e.g. Falster_2005, for Dataset citation 40).
...
Phylogenetic tree constructed by pruning  [@Smith-2018] using package `V.PhyloMaker` [@Jin-2020] and visualising using the package `ggtree` [@Yu-2017]
...
DSF was supported by Australian Research Council Future Fellowship (FT160100113), RVG was supported by Australian Research Council DECRA Fellowship (DE170100208). This work was supported by fellowship grants from Australian Research Council to Falster (FT160100113), Gallagher (DE170100208) and Wright, and funding from the Australian Research Data Commons under their "Transformative data collections" program. 
...
RVG, IJW conceived the original idea; RVG, EHW, CB collated data from primary sources; DSF developed the workflow for the harmonising of data and led all coding; EHW, DI, SCA, JL contributed to coding; EHW, CB, JL, SA error-checked trait observations; DI developed figures for the paper; DSF, RVG, DI, EHW wrote the first draft of the paper. All other authors contributed the raw data and metadata underpinning the resource and reviewed the harmonised data for errors.
...
sources_all <- 
  ...
  arrange(dataset_id) %>% 
  group_by(dataset_id) %>% 
  summarise(refs = sprintf("@%s", citation) %>% str_replace_all("; ", "; @") %>% paste(collapse = "; ") )
...
n_records <- 
  ...
  summarise(
            ...
            `Data Citations` = refs %>% unique() %>% paste(collapse = "; ") %>% paste0("[", ., "]")
            ) %>% 
