...
We achknowelde XXXXXXXXXXx institutions, who
...
![**Figure 2.** (a.) Locations where the trait data were collected and the corresponding number of taxa. (b.) AusTraits’ sites (grey) within Australia’s precipitation-temperature space (black) superimposed upon Whittaker’s major biomes diagram. Data from WorldClim (worldclim.org). (c.) Overview of the number of taxa with which traits are measured within each major plant organ and trait category.](figures/austraits_fig2_blue.png)

![**Figure 3.** The phylogenetic representation of traits in the AusTraits database for the subset of 2000 randomly sampled taxa. The heatmap colour intensities denote the number of traits measured within each major plant organ. The most widespread family names (with more than ten taxa) are labelled on the edge of the tree.](figures/austraits_phlogenetic_coverage.png)
