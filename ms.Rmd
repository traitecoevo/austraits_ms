...
to_exclude <- c("Barlow_1981","BRAIN_2007","CPRR_2002","Kew_2019_1","Kew_2019_2","Kew_2019_3",
"Kew_2019_4","Kew_2019_5","Kew_2019_6","NHNSW_2016","NTH_2014","RBGK_2014","RBGSYD_0000","RBGSYD_2014","SAH_2014", "Wheeler_2002", "Hyland_2003", "SAH_2014")

data_contributors <- austraits$contributors %>% select(-role) %>% 
    # remove authors 
    filter(!name %in% c("Peter Clarke", "Barbara Rice", "Westoby Lab")) %>% 
    # intstituional collections
    filter(!dataset_id %in% to_exclude) %>%
    select(-dataset_id) %>% 
    distinct() %>% 
    mutate(surname = str_split(name, " ") %>% map_chr(last)) %>% 
    arrange(surname) %>% rename(affiliation = institution) 


# Need to check how these institutions and individuals are to be recognised
# austraits$contributors %>% select(-role) %>% 
#     filter(dataset_id %in% to_exclude) %>% 
#     filter(name!="Rachael Gallagher") %>% 
#   write_csv("authors_check.csv")
...
We introduce the AusTraits database - a compilation of measurements of plant traits for species in the Australian Flora (hereafter AusTraits). AusTraits synthesises data on `r austraits$traits$trait_name %>% n_distinct()` traits across `r austraits$traits$species_name %>% n_distinct()` species from field campaigns, published literature, taxonomic monographs, and individual species descriptions. Traits
...
properties. This data descriptor provides information on Version `r austraits$build_info$version` of AusTraits which contains data for `r austraits$traits %>% nrow()` trait-by-species combinations. We envision AusTraits as an on-
